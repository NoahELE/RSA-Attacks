...
def generate_bin_hex_dec_key_files(key_directory, bits):
    ...
    with open(keys_paths["dec"], "w") as file, open(
        keys_paths["hex"], "w"
    ) as hexFile, open(keys_paths["bin"], "w") as bitsFile:
    ...
    # Save results to files in decimal, hexadecimal, and binary formats
    with open(keys_public_paths["dec"], "w") as file, open(
        keys_public_paths["hex"], "w"
    ) as hexFile, open(keys_public_paths["bin"], "w") as bitsFile:
        # Save in decimal format
        file.write(f"N={n.digits(10)}\n")
        file.write(f"e={e.digits(10)}\n")
...
def generate_known_bits_as_cold_start_attack(key_directory, known_bits_directory):
    ...
    with open(key_directory + "hexa-RSA-Key.txt", "r") as rsakey_file:
        for component in rsa_components:
            ...
            rsa_values[component] = read_and_convert_component(
                rsa_values[component], rsakey_file
            )
...
if __name__ == "__main__":
    ...
    if not (512 <= bits <= 2048):
        ...
        known_bits_paths = generate_known_bits_as_cold_start_attack(
            key_directory, known_bits_directory
        )
